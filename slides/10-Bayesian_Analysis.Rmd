...
* $P(PositiveTest | Cancer)$: The chances of a true positive = chance you have cancer * chance test caught it = 1% * 80% = .008
* $P(PositiveTest | NotCancer)$: The chances of a false positive = chance you don't have cancer * chance test caught it anyway = 99% * 9.6% = 0.09504

&nbsp;         | Cancer (1%)       | No Cancer (99%)      | Totals
---------------|-------------------|----------------------|-------------
...
```{r, echo=FALSE, fig.width=14, fig.height=6.5}
...
bayesplot::mcmc_trace(stan_out, pars = c("(Intercept)", "Hours"))
coefs <- as.matrix(stan_out, pars = "Hours") |> as.data.frame()
coefs$iteration <- 1:nrow(coefs)
ggplot(coefs, aes(x = iteration, y = Hours)) + geom_path()
hist(coefs)
class(coefs)
```
