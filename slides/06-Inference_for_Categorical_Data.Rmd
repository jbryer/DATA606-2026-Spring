...
density_out <- density(samp_dist, from = 0, to = 1)
density_df <- data.frame(x = density_out$x,	y = density_out$y) |>
	dplyr::mutate(area = x >= qnorm(0.025) & x <= qnorm(0.975) )
ggplot(density_df, aes(x = x, y = y)) + 
	geom_line() + 
	geom_vline(xintercept = mean(samp_dist), color = 'blue') +
	geom_errorbarh(aes(y = 0), color = 'blue',
				   xmin = mean(samp_dist) - 1.96 * sd(samp_dist),
				   xmax = mean(samp_dist) + 1.96 * sd(samp_dist)) +
	geom_vline(xintercept = mean(pop), color = 'darkgreen') +
	geom_errorbarh(aes(y = 1), color = 'darkgreen',
				   xmin = 0.85 - 1.96 * samp_se,
				   xmax = 0.85 + 1.96 * samp_se) +
	xlim(c(mean(samp_dist) - 4 * sd(samp_dist), mean(samp_dist) + 4 * sd(samp_dist)))
```
