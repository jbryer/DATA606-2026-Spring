...
* Good range of topics, as always: sports, health, travels, food, housing, education, cost…Interesting!
* Use caution when using causal language with observational data. Recommend relational language instead.
* If you submitted something aside from pdf or Rpub, we returned it without a grade, with a request to resubmit.
* If you submitted a proposal without visualizations, we did not give full credit and asked that you resubmit with visualizations included. Once you do, we will readjust your grade.
* For those with full marks, you’re on the right track. For most, we left comments in-text and general comments for you to consider. Reminder to review the rubric as you prepare for the final presentation. Some tips to keep in mind (based on previous semester oopsies):
* Include EDA — summary statistics and visualizations of your key variables. 
* Include assumptions testing (diagnostic plots)
* Be sure your analyses are focused on 1) understanding your variables in question, and 2) answering your research questions.
* If you have questions, feel free to reach out to schedule a meeting.
...
Full rubric available here: https://`r paste0(tolower(semester), year)`.data606.net/assignments/project/

[Click here to sign-up for presentation time here](`r project_signup_link`)
...
Note that the lines correspond to the value we take the log of. For "successes" (i.e. outcome = 1) then we simply take the log of the predicted probability. For "failures" (i.e outcome = 0), we take the log of 1 minus the predicted probability. 
